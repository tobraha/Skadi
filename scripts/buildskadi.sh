...
wget -O $buildskadi_tgz --quiet https://raw.githubusercontent.com/albchen/skadi/test/scripts/buildskadi.tgz
if [ $? -ne 0 ]; then
...
wget -O $buildskadi_sig --quiet https://raw.githubusercontent.com/albchen/skadi/test/scripts/buildskadi.sig
if [ $? -ne 0 ]; then
  echo "ERROR: Download was not successful. Exiting"
  rm -rf $tempdir
  exit 1
